      ...
      <div class="work-card reveal" data-category="${escapeHTML(item.category)}" data-id="${item.id}">
        ...
        <div class="work-info">
          <div>
            ...
            ${item.reel_url ? `<p style="margin-top:6px"><a href="${escapeHTML(item.reel_url)}" target="_blank" rel="noopener" style="color:inherit;text-decoration:underline">Instagram Reel ↗</a></p>` : ""}
          </div>
...
async function loadTeam(client) {
  ...
  teamGrid.innerHTML = data.map(member => {
    ...
    const photo = member.photo_url
      ? `<img src="${escapeHTML(member.photo_url)}" alt="${escapeHTML(member.name)}">`
      : initial;
